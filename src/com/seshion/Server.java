...
//							}
							...
							else if(Action=="addfriend")
							{
								...
								String result = String.valueOf(db.sendFriendRequest(aUser.getUserName(), friendname));
								encryptedByteArray = aes.encrypt(result.getBytes());
								dataNetOutputStream.write(encryptedByteArray);
							}
							else if(Action=="changeUservisibility")
							{
								aUser.changeProfileVisibility();
								String result = String.valueOf(db.changeUserVisibility(aUser));
								encryptedByteArray = aes.encrypt(result.getBytes());
								dataNetOutputStream.write(encryptedByteArray);
							}
							...
							{
								List<String> result = db.getFriends(aUser.getUserName());
								ObjectOutputStream oos = new ObjectOutputStream(bos);
								oos.writeObject(result);
								byte[] bytes = bos.toByteArray();
								encryptedByteArray = aes.encrypt(bytes);
								dataBufferedNetOutputStream.write(encryptedByteArray);
							}
							else if(Action=="removefriend")
							{
								String friendname =(String) json.get("friendname");
								String result = String.valueOf(db.removeFriend(aUser.getUserName(),friendname));
								encryptedByteArray = aes.encrypt(result.getBytes());
								dataNetOutputStream.write(encryptedByteArray);
							}
							else if(Action=="getownedsessions")
							{
								List<UserSession> result = db.getOwnedSessions(aUser.getUserName());
								ObjectOutputStream oos = new ObjectOutputStream(bos);
								oos.writeObject(result);
								byte[] bytes = bos.toByteArray();
								encryptedByteArray = aes.encrypt(bytes);
							...
							else if(Action=="getinvitedsessions")
							{
								List<UserSession> result = db.getInvitedSessions(aUser.getUserName());
								ObjectOutputStream oos = new ObjectOutputStream(bos);
								oos.writeObject(result);
								byte[] bytes = bos.toByteArray();
								encryptedByteArray = aes.encrypt(bytes);
							...
							else if(Action=="getjoinedsessions")
							{
								List<UserSession> result = db.getJoinedSessions(aUser.getUserName());
								ObjectOutputStream oos = new ObjectOutputStream(bos);
								oos.writeObject(result);
								byte[] bytes = bos.toByteArray();
								encryptedByteArray = aes.encrypt(bytes);
							...
							else if(Action=="getownedgroups")
							{
								List<UserGroup> result = db.getOwnedGroups(aUser.getUserName());
								groupList = result;
								ObjectOutputStream oos = new ObjectOutputStream(bos);
								oos.writeObject(result);
								byte[] bytes = bos.toByteArray();
							...
							else if(Action=="getjoinedgroups")
							{
								List<UserGroup> result = db.getJoinedGroups(aUser.getUserName());
								ObjectOutputStream oos = new ObjectOutputStream(bos);
								oos.writeObject(result);
								byte[] bytes = bos.toByteArray();
								encryptedByteArray = aes.encrypt(bytes);
							...
							{
								List<Message> result = db.getUserMessages(aUser.getUserName());
								ObjectOutputStream oos = new ObjectOutputStream(bos);
								oos.writeObject(result);
								byte[] bytes = bos.toByteArray();
								encryptedByteArray = aes.encrypt(bytes);
							...
							else if(Action=="createnewgroup")
							{
								...
								UserGroup group = new UserGroup(groupname, aUser.getUserName());
								groupList.add(group);
								String result = String.valueOf(db.createNewGroup(group));
								encryptedByteArray = aes.encrypt(result.getBytes());
								dataNetOutputStream.write(encryptedByteArray);
							}
							...
							{
								//String groupname = (String) json.get("groupname");
								UUID groupid = (UUID) json.get("groupid");
							...
							{
								String memberUsername = (String) json.get("memberusername");
								UUID groupid = (UUID) json.get("groupid");
								for(int i =0 ; i<groupList.size(); i++)
								{
									if(groupList.get(i).getID() == groupid) {
										String result = String.valueOf(db.addGroupMember(groupid, memberUsername));
										groupList.get(i).addGroupMember(memberUsername);
										encryptedByteArray = aes.encrypt(result.getBytes());
										dataNetOutputStream.write(encryptedByteArray);
										break;
									}
							...
							else if(Action=="removegroupmember")
							{
								...
								{
									if(groupList.get(i).getID() == groupid) {
										String result = String.valueOf(db.removeGroupMember(groupid, memberUsername));
										groupList.get(i).removeGroupMember(memberUsername);
										encryptedByteArray = aes.encrypt(result.getBytes());
										dataNetOutputStream.write(encryptedByteArray);
										break;
									}
							...
							else if(Action=="getgroupmember")
							{
								UUID groupid = (UUID) json.get("groupid");
								List<String> result = db.getGroupMembers(groupid);
								ObjectOutputStream oos = new ObjectOutputStream(bos);
								oos.writeObject(result);
								byte[] bytes = bos.toByteArray();
								encryptedByteArray = aes.encrypt(bytes);
							...
							else if(Action=="changegroupname")
							{
								...
								{
									if(groupList.get(i).getID() == groupid) {
										String result = String.valueOf(db.changeGroupName(groupid, newname));
										groupList.get(i).setName(newname);
										encryptedByteArray = aes.encrypt(result.getBytes());
										dataNetOutputStream.write(encryptedByteArray);
										break;
									}
							...
							else if(Action=="getgroupmessages")
							{
								UUID groupid = (UUID) json.get("groupid");
								for(int i =0 ; i<groupList.size(); i++)
								{
									if(groupList.get(i).getID() == groupid) {
										List<Message> result= db.getGroupMessages(groupid);
										ObjectOutputStream oos = new ObjectOutputStream(bos);
										oos.writeObject(result);
										byte[] bytes = bos.toByteArray();
										encryptedByteArray = aes.encrypt(bytes);
							...
							else if(Action=="sendmessageindividual")
							{
								LocalDate dateCreated = (LocalDate) json.get("datecreated");
								LocalTime timeCreated = (LocalTime) json.get("timecreated");
								String messageContent = (String) json.get("messagecontent");
								String recipient = (String) json.get("recipient");
								Message msg = new Message(aUser.getUserName(), recipient, dateCreated,timeCreated,messageContent,true,false);
								String result = String.valueOf(db.sendMessageIndividual(msg, recipient));
								encryptedByteArray = aes.encrypt(result.getBytes());
								dataNetOutputStream.write(encryptedByteArray);
							}
							else if(Action=="sendmessagegroup")
							{
								LocalDate dateCreated = (LocalDate) json.get("datecreated");
								LocalTime timeCreated = (LocalTime) json.get("timecreated");
								String messageContent = (String) json.get("messagecontent");
								UUID groupid = (UUID) json.get("groupid");
								Message msg = new Message(aUser.getUserName(), null, dateCreated,timeCreated,messageContent,false,true);
								String result = String.valueOf(db.sendMessageGroup(msg, groupid));
								encryptedByteArray = aes.encrypt(result.getBytes());
								dataNetOutputStream.write(encryptedByteArray);
							}
							else if(Action==" createnewsession")
							{
								String sessionname = (String) json.get("sessionname");
								double latitudeTopLeft = (double) json.get("latitudetopleft");
								double latitudeTopRight = (double) json.get("latitudetopright");
								double longitudeTopRight = (double) json.get("longitudetopright");
								double longitudeTopLeft = (double) json.get("longitudetopleft");
								double latitudeBottomLeft = (double) json.get("latitudebottomleft");
								double longitudeBottomLeft = (double) json.get("longitudebottomleft");
								double latitudeBottomRight = (double) json.get("latitudebottomright");
								double longitudeBottomRight = (double) json.get("longitudebottomright");
								LocalDate startDate = (LocalDate) json.get("startdate");
								LocalTime startTime = (LocalTime) json.get("starttime");
								boolean issessionprivate = (boolean) json.get("issessionprivate");
								UserSession session = new UserSession(sessionname,aUser.getUserName(),latitudeTopLeft,latitudeTopRight,longitudeTopRight,
										longitudeTopLeft,latitudeBottomLeft,longitudeBottomLeft,latitudeBottomRight,longitudeBottomRight,
										startDate, startTime,issessionprivate);
								sessionList.add(session);
								String result = String.valueOf(db.createNewSession(session));
								encryptedByteArray = aes.encrypt(result.getBytes());
								dataNetOutputStream.write(encryptedByteArray);
							}
							else if(Action=="cancelsession")
							{
								UUID sessionID = (UUID) json.get("sessionid");
								for(int i =0 ; i<sessionList.size(); i++)
								{
									if(sessionList.get(i).getID() == sessionID) {
										String result = String.valueOf(db.cancelSession(sessionList.get(i)));
							...
							else if(Action=="endsession")
							{
								...
								{
									if(sessionList.get(i).getID() == sessionID) {
										String result = String.valueOf(db.endSession(sessionID));
										sessionList.remove(i);
										encryptedByteArray = aes.encrypt(result.getBytes());
										dataNetOutputStream.write(encryptedByteArray);
										break;
							...
							else if(Action=="invitesessionuser")
							{
								UUID sessionID = (UUID) json.get("sessionid");
								...
								{
									if(sessionList.get(i).getID() == sessionID) {
										String result = String.valueOf(db.inviteSessionUser(sessionID,username));
										sessionList.get(i).inviteUser(username);
										encryptedByteArray = aes.encrypt(result.getBytes());
										dataNetOutputStream.write(encryptedByteArray);
										break;
									}
							...
							else if(Action=="checkinsessionuser")
							{
								UUID sessionID = (UUID) json.get("sessionid");
								String result = String.valueOf(db.checkInSessionUser(sessionID,aUser.getUserName()));
								encryptedByteArray = aes.encrypt(result.getBytes());
								dataNetOutputStream.write(encryptedByteArray);
							}
							else if(Action=="checkinsessionuser")
							{
								UUID sessionID = (UUID) json.get("sessionid");
								String result = String.valueOf(db.getAllSessionUsers(sessionID));
								ObjectOutputStream oos = new ObjectOutputStream(bos);
								oos.writeObject(result);
								byte[] bytes = bos.toByteArray();
								encryptedByteArray = aes.encrypt(bytes);
